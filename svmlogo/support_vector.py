...
class SupportVector():
    def __init__(self, sequence: str, weight: float, alphabet: int, debug: bool) -> None:
        self._debug = debug
        self._read_sequence(sequence, alphabet)  # read and evaluate support vector sequence
        self._weight = weight
    ...
    def _read_sequence(self, sequence: str, alphabet: int) -> None:
        ...
        if alphabet == DNA:
            if any(nt not in ALPHABET[DNA] for nt in sequence):
                exception_handler(ValueError, "The input sequence is not a valid DNA string", os.EX_DATAERR, self._debug)
        elif alphabet == RNA:
            if any(nt not in ALPHABET[RNA] for nt in sequence):
                exception_handler(ValueError, "The input sequence is not a valid RNA string", os.EX_DATAERR, self._debug)
    ...
    def kmers_split(self, k: int) -> Set[str]:
        ...
        exception_handler(ValueError, "Support vector sequence undefined", os.EX_DATAERR, self._debug)
